...
def twodp(x):
	...
	:param x: numeric
	:return: string of x rounded to 2 decimal places
...
def listshow(x, end=False):
    """Outputs a list as a comma separated string.
...
def namematch(a, b, z):
    """Returns an object with a matching name property from a list

	If multiple objects are matched, a random one from the list is returned.

	:param a: List of objects to search
	:param b: Item to search for
	:param z: Return value if not found
...
def playerwrite(a):
    """Creates a string output of a player's stats
...
def pitchmake(Weather):
    PaceFactor = 1 + 0.5 * (random.random() * (
                random.random() - random.random()))  # how good the pitch is for pace. 0.75-1.25, lower is better for
	# bowling
    ...
    print("Pace Factor:", "{:0.2f}".format(PaceFactor), "Spin Factor:", "{:0.2f}".format(SpinFactor),
          'Outfield Factor:', "{:0.2f}".format(Outfield))
...
def excessrpo(b):
    if b.overs == 0:
        return 0
    elif b.runs / b.overs < 4:
        return 0
    else:
        return 4 - (b.runs / b.overs)
...
def openingbowlers(a):
    ...
    if 'WK' in a.tag:
        x = x + 100
    elif 'Spin' in a.tag or 'Bat' in a.tag:
        x = x + 50
    elif 'Part' in a.tag:
        x = x + 25
    return x
...
def oversballs(x):
    if x.balls == 0:
        ...
        return str(x.overs + 1)
    else:
        return '{}.{}'.format(x.overs, x.balls)
...
class teaminnings:
    ...
    def inningsheader(self):
        ...
        if len(self.test.teaminnings) == 2 and self.test.odi == True: y = y + ' (Target: {} in {} overs)'.format(
            self.test.teaminnings[0].runs + 1, self.test.overslimit)

        if len(self.test.teaminnings) == 3 and self.test.teaminnings[1].team == self.team: y = y + ' (following on)'
        if len(self.test.teaminnings) == 4: y = y + ' (Target: {} in {} overs)'.format(self.target() + self.runs,
                                                                                       self.test.remaining() + self.overs)
        self.test.score(y)
    ...
    def run(self):
        ...
        while self.wickets < 10 and self.active == True and (
                self.test.odi == False or self.overs < self.test.overslimit):
            self.test.raincheck()
            self.over()
    ...
    def pitchfactor(self):
        if 'Spin' in self.bowler.tag:
            return self.test.pitch[1]
        else:
            return self.test.pitch[0]
    ...
    def bowlchange(self):
        ...
        if 'Part' in self.bowler.tag:
            x = x + 4
        elif 'Bat' in self.bowler.tag or 'WK' in self.bowler.tag:
            x = x + 6
        ...
        if self.overs < 5:
            x = 0
        elif quickorder(self.bowler.tag) > 2 and self.bowler.bowling.spell < 3:
            x = 0
    ...
    def bowlchoice(self, change=True):
        a = self.bowler
        if change == True:
            c = [x for x in self.bowlteam.xi if
                 x is not self.bowler and x is not self.otherbowler and x is not self.bowlteam.wk]
        else:
            c = [x for x in self.bowlteam.xi if x is not self.otherbowler and x is not self.bowlteam.wk]
        c.sort(key=lambda x: self.bowlvalue(x), reverse=False)
        if self.test.bowlingchanges == True and a is not c[0]:
            if change == True:
                self.test.logger(
                    'Bowling change: {} ({} over spell) replaced by {}'.format(a.name, a.bowling.spell, c[0].name))
            else:
    ...
    def bowlvalue(self, p):
        ...
        if 'Spin' in p.tag:
            x = x * self.det() * self.test.pitch[1]
        else:
            x = x * self.test.pitch[0]
    ...
    def det(self):
        if 'Spin' in self.bowler.tag:
            daydict = dict({1: 1.3, 2: 1.2, 3: 1.1, 4: 0.9, 5: 0.7})
        else:
            daydict = dict({1: 1.2, 2: 1.1, 3: 1, 4: 0.9, 5: 0.8})
    ...
    def aggfactor(self):
        ...
        if len(self.test.teaminnings) == 3 and self.lead() > 200 and (self.wickets < 7 or self.lead() > 250):
            x = x + (self.lead() - 200) / 200
    ...
    def howout(self):
        def catcher(x):
            y = [y for y in x.xi if y is not self.bowler and y is not self.bowlteam.wk] + [y for y in x.xi[:6] if
                                                                                           y is not self.bowler and y is not self.bowlteam.wk]
            return random.choice(y)

        x = random.random()
        if x < 0.2143:
            return 'b. {}'.format(self.bowler.name)
        elif x < 0.3573:
            return 'lbw. b. {}'.format(self.bowler.name)
        elif x < 0.52:
            ...
            return 'run out ({})'.format(random.choice(self.bowlteam.xi).name)
        elif x < 0.9999:
            return 'hit wicket b. {}'.format(self.bowler.name)
        else:
            return 'obstructed the field b. {}'.format(self.bowler.name)
    ...
    def wicket(self):
        ...
        if self.partnership(0) >= 0:
            if self.wickets != 10:
                ...
                x = ''
            self.test.logger('{} {} {} ({}b) {}x4 {}x6 - partnership {} {}'.format(self.onstrike.name,
                                                                                   self.onstrike.innings.dismissal,
                                                                                   self.onstrike.innings.runs,
                                                                                   self.onstrike.innings.balls,
                                                                                   self.onstrike.innings.fours,
                                                                                   self.onstrike.innings.sixes,
                                                                                   self.partnership(0), x))
    ...
    def ball(self):
        ...
        # print (a.name, b.name, round (self.onstrike.bat), round (self.bowler.bowl), round(z,3), round(y, 3),
		# self.aggfactor())
        ...
        if y < x < 3 * y:
            ...
            if random.random() < 1 / 10:
                ...
                self.test.logger('{} dropped by {} off {} on {}*'.format(self.onstrike.name, q.name, self.bowler.name,
                                                                         self.onstrike.innings.runs))
        ...
        if x < y:
            ...
            if len(self.overlog) >= 5 and len(self.overlog[-2]) > 3 and self.overlog[-3] == 'W' and len(
                self.overlog[-4]) > 3 and self.overlog[-5] == 'W': self.test.logger(
                'Hattrick for {}!'.format(self.bowler.name))
    ...
    def day(self):
        ...
        a = '{} {}* ({}b), {} {}* ({}b)'.format(self.batsmen[0].name, self.batsmen[0].innings.runs,
                                                self.batsmen[0].innings.balls, self.batsmen[1].name,
                                                self.batsmen[1].innings.runs, self.batsmen[1].innings.balls)
    ...
    def milestone(self, n):
        ...
        if x % 50 > 10 and self.onstrike.innings.runs % 50 <= n:
            self.test.logger('{} {}* ({}b) {}x4 {}x6'.format(self.onstrike.name, self.onstrike.innings.runs,
                                                             self.onstrike.innings.balls, self.onstrike.innings.fours,
                                                             self.onstrike.innings.sixes))
            self.declaration()
        if self.runs % 50 < n and n != 0 and self.runs >= 50:
            self.test.logger('{} {}* ({}b), {} {}* ({}b).'.format(self.batsmen[0].name, self.batsmen[0].innings.runs,
                                                                  self.batsmen[0].innings.balls, self.batsmen[1].name,
                                                                  self.batsmen[1].innings.runs,
                                                                  self.batsmen[1].innings.balls))
            self.declaration()

    def partnership(self, n):
        try:
            a = max([x.FOW[1] for x in self.innings if x.FOW is not ''])
        except:
            a = 0
        y = self.runs - a
        if y % 50 < n and y >= 50:
    ...
    def declaration(self):
        ...
        if len(self.test.teaminnings) == 1 and (
                self.runs > 500 or self.overs > 150) and random.random() < self.runs / 50000:
            self.declare = True
        elif len(self.test.teaminnings) == 2 and self.runs > 500 and self.lead() > -50 and random.random() < 0.01:
            self.declare = True
        elif len(self.test.teaminnings) == 2 and (self.runs / self.test.teaminnings[
            0].runs > 2) and self.overs > 100 and self.runs > 350 and random.random() < 0.01:
            self.declare = True
        elif len(self.test.teaminnings) == 2 and self.lead() > 200 and self.runs > 400 and random.random() < 0.01:
            self.declare = True
        elif len(self.test.teaminnings) == 3 and self.lead() > (
                self.test.remaining() * self.test.rpo + 100) and self.test.remaining() > 35:
            self.declare = True
    ...
    def allout(self):
        ...
        if len(self.test.teaminnings) < 4 or self.test.win != '':
            ...
            self.test.logger('End of innings - {} {}{} ({}b), {} {}{} ({}b).'.format(self.batsmen[0].name,
                                                                                     self.batsmen[0].innings.runs, q,
                                                                                     self.batsmen[0].innings.balls,
                                                                                     self.batsmen[1].name,
                                                                                     self.batsmen[1].innings.runs, w,
                                                                                     self.batsmen[1].innings.balls))
        ...
        for a in self.innings:
            ...
            if self.test.fullcard == True:
                a.scorecard = '{}{} {} {} {} FOW:{}      {}'.format(tag.rjust(1), name, a.dismissal.rjust(50),
                                                                    str(a.runs).rjust(3),
                                                                    '({})'.format(a.balls).rjust(5), FOW.rjust(6),
                                                                    a.player.batdesc(self.test.year))
            else:
                a.scorecard = '{}{} {} {} {} FOW:{}'.format(tag.rjust(1), name, a.dismissal.rjust(50),
                                                            str(a.runs).rjust(3), '({})'.format(a.balls).rjust(5),
                                                            FOW.rjust(6))
        ...
        if self.overs + self.balls > 0:
            z = "%.2f" % (self.runs / (self.overs + self.balls / 6))
        else:
            z = ''
        ...
        for i in self.order:
            ...
            if o > 0:
                e = "%.2f" % round(i.runs / o, 2)
            else:
                e = "----"

            if self.test.fullcard == True:
                y = ' {} {} - {} - {} - {}   {} RPO {} {} {}'.format(i.name.ljust(20), oversballs(i).center(4),
                                                                     str(i.maidens).rjust(2), str(i.runs).rjust(3),
                                                                     str(i.wickets).rjust(2), str(e),
                                                                     i.player.tag.center(15), ''.ljust(28),
                                                                     i.player.bowldesc(self.test.year))
            else:
                y = ' {} {} - {} - {} - {}   {} RPO {}'.format(i.name.ljust(20), oversballs(i).center(4),
                                                               str(i.maidens).rjust(2), str(i.runs).rjust(3),
                                                               str(i.wickets).rjust(2), str(e), i.player.tag.center(15))
            self.test.score(y)
    ...
    def followon(self):
        ...
        if len(self.test.teaminnings) == 2 and self.lead() <= -200 and self.wickets == 10:
            if (self.test.weather == 'Timeless' and self.lead() <= -300) or -(
            self.lead()) > self.test.remaining() - self.test.lostovers + self.overs * 0.5:
    ...
    def endofgame(self):
        ...
        if self.test.win in ['Draw', 'Tie']:
            for i in self.test.teaminnings:
                x = [i.runs, i.wickets]
                if i.team == self.test.teaminnings[0].team:
                    a.append(scoreformat(x))
                else:
                    b.append(scoreformat(x))

            if self.test.win == 'Draw':
                x = ' {} ({}) drew with {} ({})'.format(self.test.teaminnings[0].team.name, listshow(a),
                                                        [x for x in [self.test.home, self.test.away] if
                                                         x is not self.test.teaminnings[0].team][0].name, listshow(b))
            else:
                x = ' {} ({}) tied with {} ({})'.format(self.test.teaminnings[0].team.name, listshow(a),
                                                        [x for x in [self.test.home, self.test.away] if
                                                         x is not self.test.teaminnings[0].team][0].name, listshow(b))
            ...
            for i in self.test.teaminnings:
                x = [i.runs, i.wickets]
                if i.team == self.test.win:
                    a.append(scoreformat(x))
                else:
                    b.append(scoreformat(x))
            x = ' {} ({}) beat {} ({}) by {}'.format(self.test.win.name, listshow(a),
                                                     [x for x in [self.test.home, self.test.away] if
                                                      x is not self.test.win][0].name, listshow(b), self.test.margin)
        self.test.score(x)
        self.test.resultdesc = x
        if self.test.win in ['Draw', 'Tie']:
            self.test.winname = self.test.win
        else:
            self.test.winname = self.test.win.name
        ...
        for i in self.test.teaminnings:
            ...
            for j in i.order:
                if j.player == self.test.motm and ((j.balls > 0) or (j.overs > 0)): x.append(
                    '{}-{}'.format(j.wickets, j.runs))
        ...
        if any(
            (x.runs >= 700 or (x.runs <= 50 and x.wickets == 10)) for x in self.test.teaminnings): self.test.cardsave()
        if any((x.runs >= 250 or (x.runs >= 200 and (x.runs / x.balls > 1 or self.test.year < 1900))) for x in
               self.test.inns): self.test.cardsave()
    ...
    def result(self):
        ...
        if self.test.remaining() <= 0 and (
                (b >= a and self.wickets < 10) or (len(self.test.teaminnings) < 4 and a >= b)):
...
class test:
    ...
    def conditions(self):
        ...
        if self.weather == 'Timeless':
            print('Timeless Test')
            self.gameovers = self.scheduledovers = random.choice(
                [900, 990, 1080, 1170, 1260, 1350, 1440, 1530, 1620, 1710, 1800])
            if random.random() < 0.02:
                self.gameovers = self.scheduledovers * (1 - (random.random() * random.random() * random.random()))
            elif random.random() < 0.15:
                self.gameovers = random.randrange(round(0.8 * self.scheduledovers), self.scheduledovers)
            ...
            if random.random() < 0.03:
                self.gameovers = self.scheduledovers * (1 - (random.random() * random.random()))
            elif random.random() < 0.15:
                self.gameovers = random.randrange(round(0.8 * self.scheduledovers), self.scheduledovers)
        ...
        with open('settings.txt') as f:
            ...
            if any(x[0] == 'active' and x[1] == 'true' for x in a):
                for i in a:
                    ...
                    print(i[0], '-', i[1])
                    if i[0] == 'pace':
                        self.pitch[0] = float(i[1])
                    elif i[0] == 'spin':
                        self.pitch[1] = float(i[1])
                    elif i[0] == 'outfield':
                        self.pitch[2] = float(i[1])
                    elif i[0] == 'venue':
                        self.venue = i[1]
                    elif i[0] == 'year':
                        self.year = int(i[1])
                        ...
                        self.home.gamecapt = namematch(self.home.xi, i[1], self.home.gamecapt)
                    elif i[0] == 'homewk':
                        self.home.wk = namematch(self.home.xi, i[1], self.home.wk)
                    elif i[0] == 'awaywk':
                        self.away.wk = namematch(self.away.xi, i[1], self.away.wk)
                    elif i[0] == 'awaycapt':
                        self.away.gamecapt = namematch(self.home.xi, i[1], self.away.gamecapt)
                    elif i[0] == 'toss':
                        if i[1] == 'home':
                            self.tosswin = self.home
                        elif i[1] == 'away':
                            self.tosswin = self.away
    ...
    def decision(self):
        if self.choice == '':
            if random.random() < 0.8 or (random.random() < 0.5 and self.odi == True):
                self.choice = True
            else:
                self.choice = False

        if self.choice == True:
            self.score('{} has elected to bat first.'.format(self.tosswin.gamecapt.name))
        else:
            self.score('{} has elected to field first.'.format(self.tosswin.gamecapt.name))
    ...
    def toss(self):
        ...
        self.decision()
        if self.choice == True:
            self.tobat = self.tosswin
        else:
            self.tobat = [y for y in [self.home, self.away] if y is not self.tosswin][0]

    def overcount(self):
        return sum([x.overs for x in self.teaminnings]) + sum(
            [x.balls > 0 for x in self.teaminnings if x is not self.teaminnings[-1]]) + self.lostovers

    def raincheck(self):
        if self.odi == True: return

        if self.lostovers >= self.scheduledovers - self.gameovers:
            return
        elif random.random() < (self.scheduledovers - self.gameovers) / (10 * self.scheduledovers):
            x = random.randrange(0, 10) + round(
                random.random() * (self.scheduledovers - self.gameovers - self.lostovers))
            x = min(x, self.scheduledovers - self.gameovers - self.lostovers)
            if x > 0:
                self.logger('{} overs lost due to bad weather.'.format(x))
                self.logger('')
                # self.logger('scheduledovers {} gameovers {} lostovers {}'.format(self.scheduledovers,
				# self.gameovers, self.lostovers+x))
                ...
                if len(self.teaminnings[-1].fatigue) > x:
                    for i in range(x):
                        self.teaminnings[-1].fatigue.pop(0)
                    else:
                        self.teaminnings[-1].fatigue = []
    ...
    def logger(self, x):
        ...
        if len(self.teaminnings) == 1:
            x.append('')
        elif len(self.teaminnings) in [2, 3]:
            if n > 0:
                x.append(' lead by {}'.format(n))
            elif n == 0:
                x.append(' scores level')
            else:
                x.append(' trail by {}'.format(-n))
        else:
            x.append(' target {}'.format(self.teaminnings[3].target() + self.teaminnings[3].runs))
    ...
    def cardsave(self):
        try:
            ...
            if self.fullcard == True:
                y = '{}, {} v. {} at {}, {} {}\n'.format(self.raw[4], self.raw[1], self.raw[2], self.raw[3],
                                                         self.raw[7], self.raw[8])
                # c.insert(0,y)
                if len(self.series.results) > 0:
                    d = ['Series:\n']
                else:
                    d = []
                for t in self.series.results:
                    if t.win in ['Draw', 'Tie']:
                        z = t.win
                    else:
                        z = t.win.name + ' won by ' + t.margin
    ...
    def gamedesc(self):
        # print ((self.home.name, 'v.', self.away.name).ljust(35), 'at', self.venue.ljust(15), "| Test #",
		# str(self.no).ljust(4),'|', self.dates, '|', y, '| MOTM:', self.motm.name)
        print('{} {} Test #{} {} MOTM: {}'.format(self.resultdesc.center(75), self.venue.center(20),
                                                  str(self.no).ljust(4), self.dates.center(30), self.motm.name))
    ...
    def motmscore(self, p):
        ...
        x = x + sum([i.wickets for i in a]) * self.base + sum([i.overs for i in a]) * self.rpo * 0.5 - sum(
            [i.runs for i in a])
        ...
        if self.win in ['Draw', 'Tie']:
            pass
        elif p in self.win.xi:
            x += 100
    ...
    def pitchdetails(self):
        return ' Pace Factor: {}, Spin Factor: {}, Outfield Speed: {}'.format("{:0.2f}".format(self.pitch[0]),
                                                                              "{:0.2f}".format(self.pitch[1]),
                                                                              "{:0.2f}".format(self.pitch[2]))

    def pitchreport(self):
        a = ['appeared to be', 'looked to be', 'seemed', 'was', 'was described as being']

        if self.pitch[0] < 0.75:
            b = 'a minefield.'
        elif self.pitch[1] < 0.8:
            b = "a spinner's paradise."
        elif self.pitch[0] < 0.9 and self.pitch[1] < 0.9:
            b = 'friendly to bowlers.'
        elif all(x > 1.2 for x in self.pitch):
            b = 'extremely flat.'
        elif all(x > 1.1 for x in self.pitch):
            b = 'very flat.'
        elif all(x > 1 for x in self.pitch):
            b = 'somewhat flat.'
        elif self.pitch[2] < 0.9 and self.pitch[1] > 1.1 and self.pitch[0] > 1.1:
            b = 'dead.'
        elif self.pitch[2] < 0.9 and self.pitch[1] > 1 and self.pitch[0] > 1:
            b = 'slow.'
        else:
            b = 'fair.'
    ...
    def matchreport(self):
        ...
        if self.win not in ['Draw', 'Tie'] and self.loss.name == 'England' and 'innings' in self.margin:
            a.append('Well, Charles...')
        elif any(x.team.name == 'England' and x.runs < 100 and x.wickets == 10 and self.year > 1900 for x in
                 self.teaminnings):
            a.append('Well, Charles...')

        if self.tosswin == self.teaminnings[0].team:
            a.append('{} of {} won the toss and batted first.'.format(self.tosswin.gamecapt.name, self.tosswin.name))
        else:
            a.append('{} won the toss and fielded first.'.format(self.tosswin.gamecapt.name))

        if len(self.teaminnings) == 4 and self.win == self.teaminnings[1].team == self.teaminnings[2].team: a.append(
            'The game was a classic.')

        a.append(self.pitchreport())

        if self.win == 'Draw' and self.lostovers > 90:
            a.append('Unfortunately, the game was spoiled by rain.')
        elif self.win == 'Draw' and self.lostovers > 30:
            a.append('The game was affected by rain.')
        if sum([x.runs for x in self.teaminnings]) > 1800:
            a.append('It was a very high-scoring game.')
        elif sum([x.runs for x in self.teaminnings]) > 1400:
            a.append('It was a high-scoring game.')
        elif sum([x.runs for x in self.teaminnings]) < 600 and len(self.teaminnings) > 2:
            a.append('It was a very low-scoring game.')

        for i in self.teaminnings:
            if i == self.teaminnings[0]:
                x = 'In the first innings,'
            elif i == self.teaminnings[1]:
                x = 'In reply,'
            elif i == self.teaminnings[2]:
                x = 'Batting third,'
            ...
            a.append(
                '{} {} {} {}{} in {} overs.'.format(x, i.team.name, random.choice(z), scoreformat([i.runs, i.wickets]),
                                                    y, oversballs(i)))
            ...
            if len(self.teaminnings) > 1 and i == self.teaminnings[1]:
                if i.runs > self.teaminnings[0].runs:
                    a.append('meaning that {} led by {}.'.format(i.team.name, i.runs - self.teaminnings[0].runs))
                elif i.runs < self.teaminnings[0].runs:
                    a.append('so {} trailed by {}.'.format(i.team.name, self.teaminnings[0].runs - i.runs))
                else:
                    a.append('so the scores were level.')

            if len(self.teaminnings) > 2 and i == self.teaminnings[1] and i.team == self.teaminnings[2].team: a.append(
                '{} were forced to follow on.'.format(i.team.name))
...
class innings:
    ...
    def report(self, x):
        ...
        if self.runs == max([q.runs for q in
                             x.innings]) and self.runs < 150 and random.random() < 0.1: return '{} top scored with {}'.format(
            self.name, score)
        if self.runs % 100 > 90 and self.out == False and random.random() < 1 / 3: return '{} was stranded on {}'.format(
            self.name, score)
        ...
        if SR > 0.75:
            a = a + ['hard-hitting', 'dominant', 'brisk', 'very attacking', 'fluid']
        elif SR < 0.4:
            a = a + ['dogged', 'determined', 'grinding', 'slow', 'tough', ]
        ...
        if SR > 1: v = v + ['slogged']
        if SR > 0.8:
            v = v + ['smashed', 'crunched', 'crushed', 'struck', 'hit']
        elif SR < 0.5:
            v = v + ['compiled', 'grinded to', 'battled for', 'worked for', 'managed']

        if a == []:
            x = '{} {} {}'.format(self.name, random.choice(v), score)
        elif SR > 1 or ((SR > 0.75 or SR < 0.4) and random.random() < 1 / 3):
            return '{} {} a {} {} from {} balls'.format(self.name, random.choice(v), random.choice(a), score,
                                                        self.balls)

        if random.random() < 1 / 3 and self.out == True:
            if 'c.' in self.dismissal:
                howout = 'caught'
            elif 'lbw.' in self.dismissal:
                howout = 'LBW'
            elif 'st.' in self.dismissal:
                howout = 'stumped'
            elif 'run out' in self.dismissal:
                howout = 'run out'
            else:
                howout = 'bowled'
        ...
        if self.player.games == 1 and self.test.fullcard == True and self.player.team == 'Australia' and random.random() < 1 / 1000:
            x = x + " on dayboo"
        elif self.player.games == 1 and self.test.fullcard == True and random.random() < 1 / 3:
            x = x + ' on debut'

        return x

    def form(self):
        if self.test.overcount() % 90 < 6 or self.test.teaminnings[-1].overs < 10 and 'Open' not in self.player.tag:
            return 0.95
        else:
            return 0.95 + min(0.1, self.balls / 2000) + min(0.05, self.runs / 1000)  # - min (0, (200-self.balls)/2000)

    def stats(self):
        self.player.inns.append(self)
        self.test.inns.append(self)
        if 100 > self.runs >= 50:
            self.player.fifties += 1
        elif self.runs >= 100:
            self.player.centuries += 1
        if self.runs >= 200: self.player.doubles += 1
        if self.runs > self.player.HS or (
                self.runs == self.player.HS and self.player.HSNO == ' ' and self.out == False):
    ...
    def desc(self):
        if self.out == True:
            ...
            x = '*'
        return '{}{} ({}) {}x4{}x6'.format(str(self.runs).rjust(3), x, str(self.balls).rjust(3),
                                           str(self.fours).rjust(2), str(self.sixes).rjust(2))
...
class bowling:
    ...
    def report(self, x):
        ...
        a = []
        if self.wickets > 3:
            a = a + ['well', 'effectively', 'smartly', 'well', 'hard']
        elif random.random() < 0.5:
            return '{} took {}-{}'.format(self.name, self.wickets, self.runs)
        if self.wickets >= 5:
            a = a + ['beautifully', 'wonderfully', 'magnificantly']
        elif self.overs >= 30:
            a = a + ['tirelessly', 'relentlessly', 'all day', 'without complaint', 'hard for his captain']
        ...
        x = '{} {} {}, {} {}-{}'.format(self.name, random.choice(v), random.choice(a), random.choice(b), self.wickets,
                                        self.runs)
    ...
    def stats(self):
        ...
        if any((self.player == y.player and self.wickets + y.wickets >= 10 and self is not y) for y in
               self.test.bowls): self.player.tens += 1
...
class player:
    ...
    def stats(self, test):
        ...
        if self.outs > 0:
            self.batav = round(self.runs / self.outs, 2)
        else:
            self.batav = self.runs
        ...
        if self.ballsbowled > 0:
            ...
            if self.ballsbowled % 6 == 0:
                self.overs = str(self.ballsbowled / 6)
            else:
                self.overs = '{}.{}'.format(self.ballsbowled // 6, self.ballsbowled % 6)

    def batdesc(self, current):
        if self.end + 1 < current:
            y = '{}-{}'.format(self.debut, self.end)
        else:
            y = '{}-    '.format(self.debut)
        return 'Age{} {} {} Tests {} runs @{} SR{}{}x200{}x100{}x50 HS {}{}{}ct{}st Rating{}'.format(
            str(current - self.dob).rjust(3), y, str(self.games).rjust(3), str(self.runs).rjust(5),
            "{:0.2f}".format(self.batav).rjust(6), "{:0.1f}".format(self.batsr).rjust(5), str(self.doubles).rjust(2),
            str(self.centuries).rjust(3), str(self.fifties).rjust(3), str(self.HS).rjust(3), self.HSNO,
            str(self.catches).rjust(3), str(self.stumpings).rjust(2), "{:0.2f}".format(self.batform).rjust(6))

    def bowldesc(self, current):
        if self.BBR == float('inf'):
            BB = '-'.center(6)
        else:
            BB = str(self.BBW).rjust(2) + '-' + str(self.BBR).ljust(3)

        if self.end == 9999:
            y = '         '
        elif self.end + 1 < current:
            y = '{}-{}'.format(self.debut, self.end)
        else:
            y = '{}-    '.format(self.debut)
        return ('Age{} {} {} Tests {} wickets @{} {} overs {}x5w {}x10w BB{} SR{} ER {} Rating {}'.format(
            str(current - self.dob).rjust(3), y, str(self.games).rjust(3), str(self.wickets).rjust(3),
            "{:0.2f}".format(self.bowlav).rjust(6), str(self.overs).rjust(6), str(self.fives).rjust(2),
            str(self.tens).rjust(2), BB, "{:0.1f}".format(self.bowlsr).rjust(5), "{:0.2f}".format(self.bowler).rjust(4),
            "{:0.2f}".format(self.bowlform).rjust(6)))
    ...
    def shortbowldesc(self):
        if self.BBR == float('inf'):
            ...
            BB = str(self.BBW).rjust(2) + '-' + str(self.BBR).ljust(3)
        return ('{} wickets @{} {}x5w {}x10w BB{} SR{} ER {} Rating{}'.format(str(self.wickets).rjust(3),
                                                                              "{:0.2f}".format(self.bowlav).rjust(6),
                                                                              str(self.fives).rjust(2),
                                                                              str(self.tens).rjust(2), BB,
                                                                              "{:0.1f}".format(self.bowlsr).rjust(5),
                                                                              "{:0.2f}".format(self.bowler).rjust(4),
                                                                              "{:0.2f}".format(self.bowlform).rjust(6)))
    ...
    def statscsv(self):
        ...
        x = x + [self.runs, self.balls, self.batav, self.batsr, self.doubles, self.centuries, self.fifties, self.HS,
                 self.HSNO]
        x = x + [self.ballsbowled, self.maidens, self.bowlruns, self.wickets, self.bowlav, self.bowler, self.bowlsr,
                 self.fives, self.tens, self.BBR, '-', self.BBW]
